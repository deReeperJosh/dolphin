...
void Skylander::Save()
{
  if (!sky_file)
  {
    ...
    File::IOFile new_sky_file(file_path, "w+b");
    if (!new_sky_file)
    {
      return;
    }
    ...
    {
      sky_file = std::move(new_sky_file);
    }
  }
...
u8 SkylanderPortal::LoadSkylander(std::unique_ptr<SkylanderFigure> figure, std::string file_path)
{
  std::lock_guard lock(sky_mutex);
...
std::pair<u16, u16> SkylanderPortal::GetSkylanderFromSlot(u8 slot)
{
  ...
  return CalculateIDs(skylanders[slot].data);
}
